...
public class NoOfProductsServiceImp implements NoOfProductsService
{
	...
	public List<NoOfProducts> getNoOfProducts(int product_id) 
	{
		try
		{
			return sessionFactory.getCurrentSession().createQuery("from NoOfProducts where products_product_id=:id",NoOfProducts.class).setParameter("id",product_id).getResultList();			
		} 
		catch (Exception e) 
		{
			return null;
